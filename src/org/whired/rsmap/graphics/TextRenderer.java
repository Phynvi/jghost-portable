...
public class TextRenderer {
	...
	public void render(final String s, int x, int y, final int k) {
		if (s == null) {
			return;
		}
		y -= maxCharHeight;
	...
	public TextRenderer(final RSCanvas canvas, final CacheLoader cacheLoader, final String font) {
		this.canvas = canvas;
		...
		maxCharHeight = 0;
		final ByteBuffer nodeBuffer = new ByteBuffer(cacheLoader.loadNode(font + ".dat"));
		final ByteBuffer indexBuffer = new ByteBuffer(cacheLoader.loadNode("index.dat"));
		indexBuffer.currentOffset = nodeBuffer.getShort() + 4;
		final int bytesToSkip = indexBuffer.getUnsignedByte();
		if (bytesToSkip > 0) {
		...
		for (int curChar = 0; curChar < 256; curChar++) {
			...
			final int curCharWidth = charWidths[curChar] = indexBuffer.getShort();
			final int curCharHeight = charHeights[curChar] = indexBuffer.getShort();
			final int readMode = indexBuffer.getUnsignedByte();
			final int curCharPixels = curCharWidth * curCharHeight;
	...
	public void renderText(final String text, final int i, final int j, final int k) {
		render(text, i - getTextWidth(text) / 2, j, k);
	}

	public void attemptRender(final byte abyte0[], int i, int j, int k, int l, final int i1) {
		int j1 = i + j * canvas.getWidth();
		int k1 = canvas.getWidth() - k;
		int l1 = 0;
		int i2 = 0;
		if (j < canvas.startY) {
			final int j2 = canvas.startY - j;
			l -= j2;
			j = canvas.startY;
			i2 += j2 * k;
		...
		if (i + k >= canvas.endX) {
			final int l2 = i + k - canvas.endX + 1;
			k -= l2;
			l1 += l2;
			k1 += l2;
		}
	...
	public void renderText(final byte abyte0[], final int i, int j, int k, int l, final int i1, final int j1, final int k1) {
		final int l1 = -(l >> 2);
		l = -(l & 3);
