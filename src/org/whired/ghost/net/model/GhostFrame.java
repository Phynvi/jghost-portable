...
/**
 * Provides the functionality and layout for a standard implementation of a
 * JFrame that will utilize Ghost's core functionality
 * 
 * @author Whired
 */
public abstract class GhostFrame implements Receivable, AbstractClient {
	...
	public boolean handlePacket(int packetId, int packetLength, Connection connection) {
		...
		break;
		default:
			// Notify whatever higher listener that they are to handle this
			// packet
			...
			if (packet != null)
				if (packet.receive(connection))
					...
					return false;
			// return this.getUser().handlePacket(packetId, packetLength,
			// connection);
		}
		return true;
	}
	...
	public void packetReceived(GhostPacket packet) {
		// TODO Remove? Dead code piling up..
	}

	/**
	 * Displays the window that allows the user to select a chain of accessors
	 * that will result in the formation of a reflection packet
	 */
	protected void displayReflectionManager() {
		throw new UnsupportedOperationException("Momentarily disabled.");
		// if (this.getConnection() != null) {
		// reflectionPacketBuilderManager = new
		// ReflectionPacketBuilderManager((JFrame) this.getOwner(),
		// this.getConnection());
